...
const nodeActions = (node: SoapNode) => {
  ...
  if (kind === 'reactor' || kind === 'heatedReactor') return [
    ['Включить', 'reactor:start'], ['Остановить', 'reactor:stop'], ['Ожидание', 'reactor:setIdle'], ['Ремонт', 'reactor:setMaintenance'],
  ] as const;
  ...
  if (kind === 'fillingStation') return [
    ['Разрешить приём', 'station:enableIntake'], ['Запретить приём', 'station:disableIntake'],
  ] as const;
...
export const InspectorPanel = ({ compact = false }: { compact?: boolean }) => {
  ...
  if (!node && !edge) return <aside className={`panel inspector-panel ${compact ? 'is-compact' : ''}`}><div className="panel-title">Инспектор</div><p className="empty-state">Выберите оборудование или сегмент трубопровода, чтобы управлять параметрами и инженерными действиями.</p><DiagnosticsSection /></aside>;
