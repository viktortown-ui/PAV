...
export const TopToolbar = ({ focusMode, onToggleFocusMode, onToggleLibrary, onOpenCommandPalette }: TopToolbarProps) => {
  ...
  return <header className="top-toolbar top-toolbar-refactor"><div className="brand brand-compact"><span>Конструктор P&ID ПАВ</span><small>Canvas-first shell • меньше chrome, больше схемы</small></div><div className="toolbar-clusters toolbar-clusters-compact"><div className="toolbar-group toolbar-group-quick"><button className="primary" onClick={onOpenCommandPalette}>⌘/Ctrl K</button><button onClick={() => void rf.fitView({ padding: 0.22, duration: 250 })}>Вписать</button><button onClick={onToggleLibrary}>{focusMode ? 'Библиотека' : 'Левая панель'}</button><button onClick={onToggleFocusMode}>{focusMode ? 'Выход из focus' : 'Focus mode'}</button></div><div className="toolbar-group toolbar-group-compact-actions"><details className="toolbar-menu"><summary>Файл</summary><div className="toolbar-menu-sheet"><button onClick={() => void loadProject()}>Открыть проект</button><button className="primary" onClick={() => void saveProject('manual')}>Сохранить</button><button onClick={onExport}>Экспорт JSON</button><label className="import-button">Импорт JSON<input type="file" accept="application/json" onChange={(e) => e.target.files?.[0]?.text().then(importProject)} hidden /></label></div></details><details className="toolbar-menu"><summary>Шаблоны</summary><div className="toolbar-menu-sheet"><button onClick={() => loadTemplate('water-prep')}>Шаблон воды</button><button onClick={() => loadTemplate('soap-line')}>Шаблон ПАВ</button><button onClick={() => loadTemplate('cip-fragment')}>Шаблон CIP</button></div></details><details className="toolbar-menu"><summary>Вид</summary><div className="toolbar-menu-sheet"><label className="toolbar-select"><span>Подписи линий</span><select value={edgeLabelMode} onChange={(e) => setEdgeLabelMode(e.target.value as EdgeLabelMode)}>{edgeLabelModes.map((mode) => <option key={mode.value} value={mode.value}>{mode.label}</option>)}</select></label><button onClick={runValidation}>Проверить схему</button></div></details><details className="toolbar-menu"><summary>Сервис</summary><div className="toolbar-menu-sheet"><button onClick={() => void resetProject()}>Сбросить проект</button><button onClick={() => void resetUserData()}>Сбросить данные пользователя</button><button onClick={() => void clearLocalDataAndLoadDemo()}>Полный сброс версии</button></div></details></div></div></header>;
};
