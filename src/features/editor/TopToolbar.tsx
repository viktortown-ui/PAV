...
export const TopToolbar = ({ focusMode, onToggleFocusMode, onOpenLibrary, onOpenCommandPalette }: TopToolbarProps) => {
  ...
  const persistenceLabel = persistenceError
    ? persistenceError
    : persistenceStatus === 'saving'
      ? 'Сохраняется…'
      : hasUnsavedChanges || persistenceStatus === 'dirty'
  ...
  const onExport = useCallback(() => {
    ...
    link.href = url;
    link.download = 'pav-project.json';
    link.click();
    URL.revokeObjectURL(url);
  }, [exportProject]);

  return (
    <header className="top-toolbar top-toolbar-refactor">
      <div className="brand brand-compact">
        <span>Конструктор техсхем</span>
        <small>Редактор технологических схем для оператора и инженера.</small>
      </div>
      <div className="toolbar-clusters toolbar-clusters-compact">
        ...
        <div className="toolbar-group toolbar-group-mode-switch" role="tablist" aria-label="Режим отображения">
          <button
            type="button"
            role="tab"
            aria-selected={presentationMode === 'schematic'}
            className={presentationMode === 'schematic' ? 'is-active' : ''}
            onClick={() => setPresentationMode('schematic')}
          >
            Схема
          </button>
          <button
            type="button"
            role="tab"
            aria-selected={presentationMode === 'simulation'}
            className={presentationMode === 'simulation' ? 'is-active' : ''}
            onClick={() => setPresentationMode('simulation')}
          >
            Симуляция
          </button>
        </div>

        <div className="toolbar-group toolbar-group-compact-actions">
          <details className="toolbar-menu">
            <summary>Файл</summary>
            <div className="toolbar-menu-sheet">
              <button onClick={() => void loadProject()}>Открыть проект</button>
              <button className="primary" onClick={() => void saveProject('manual')}>Сохранить</button>
              <button onClick={onExport}>Экспорт JSON</button>
              <label className="import-button">
                Импорт JSON
                <input type="file" accept="application/json" onChange={(e) => e.target.files?.[0]?.text().then(importProject)} hidden />
              </label>
            </div>
          </details>
          <details className="toolbar-menu">
            ...
            <div className="toolbar-menu-sheet">
              <button onClick={() => loadTemplate('water-prep')}>Водоподготовка</button>
              <button onClick={() => loadTemplate('soap-line')}>Линия ПАВ</button>
              <button onClick={() => loadTemplate('cip-fragment')}>CIP-контур</button>
            </div>
          </details>
          <details className="toolbar-menu">
            <summary>Вид</summary>
            <div className="toolbar-menu-sheet">
              ...
              <label className="toolbar-select">
                ...
                <select value={edgeLabelMode} onChange={(e) => setEdgeLabelMode(e.target.value as EdgeLabelMode)}>
                  {edgeLabelModes.map((mode) => <option key={mode.value} value={mode.value}>{mode.label}</option>)}
                </select>
              </label>
          ...
          <details className="toolbar-menu">
            ...
            <div className="toolbar-menu-sheet">
              <button onClick={() => void resetProject()}>Очистить схему</button>
              <button onClick={() => void resetUserData()}>Очистить локальные данные</button>
              <button onClick={() => void clearLocalDataAndLoadDemo()}>Восстановить демо-схему</button>
            </div>
          </details>
          <span className={`toolbar-save-indicator status-${persistenceStatus}`} role="status">{persistenceLabel}</span>
        </div>
      </div>
    </header>
  );
