...
const sanitizeNode = (value: unknown): SoapNode | null => {
  ...
  const sanitizedData: SoapNodeData = {
    ...
    orientation: value.data.orientation === 'vertical' ? 'vertical' : value.data.orientation === 'horizontal' ? 'horizontal' : definition.defaults.orientation,
    symbolVariant: typeof value.data.symbolVariant === 'string' ? value.data.symbolVariant : undefined,
...
const sanitizeEdge = (value: unknown, nodeIds: Set<string>): SoapEdge | null => {
  ...
  return {
    ...
    data: {
      ...
      sourcePortId: typeof edgeData.sourcePortId === 'string' ? edgeData.sourcePortId : undefined,
      targetPortId: typeof edgeData.targetPortId === 'string' ? edgeData.targetPortId : undefined,
