...
const routeStateLabel: Record<RouteState, string> = {
  idle: 'Линия ожидания',
  primed: 'Подготовлен',
  flowing: 'Поток',
  blocked: 'Блокировка',
...
const mediumLabel: Record<MediumType, string> = {
  water: 'Вода',
  product: 'Продукт',
  cip: 'CIP',
  waste: 'Сток',
...
export const FlowEdge = ({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, data, selected }: EdgeProps) => {
  ...
  const showAtZoom = zoom >= 0.72;
  const isZeroIdle = !active && !blocked && Number(data?.flowRate ?? 0) <= 0 && routeKey === 'idle';
  const isMeaningful = active || blocked || !isZeroIdle;

  const shouldShowBadge = showAtZoom && (() => {
    if (visibleMode === 'hidden') return false;
    if (visibleMode === 'all') return isMeaningful || emphasis || hovered;
    if (visibleMode === 'active') return active || blocked;
    return selected || hovered || emphasis;
  })();

  const stateLabel = blocked ? 'Блокировка' : routeStateLabel[routeKey];
  const meta = [stateLabel];
  if (active || blocked || hovered || selected) meta.push(mediumLabel[mediumKey]);
      ...
      {shouldShowBadge ? (
        <EdgeLabelRenderer>
          <div
            ...
            className={`edge-badge ${emphasis ? 'is-focus' : ''} ${active ? 'is-active' : ''} ${blocked ? 'is-warning' : ''}`}
          >
