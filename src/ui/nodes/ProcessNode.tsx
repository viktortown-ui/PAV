  ...
  return (
    <div className={[
      ...
      <div className="node-shell">
        <div className={`node-icon ${isRunning ? 'is-live' : ''} ${agitatorOn ? 'is-mixing' : ''}`}>
        ...
        <div className="node-copy">
          ...
          {!vesselLike && !far && (lineEquipment || microInline) ? <div className="node-inline-meta inline-state"><span>{routeLabel[routeState]}</span>{data.className === 'valve' ? <span>{valveOpen ? 'Открыт' : 'Закрыт'}</span> : null}{isAlarmed ? <span>Тревога</span> : null}</div> : null}
        </div>
      ...
      {showInlineInspectorMeta && <div className="node-summary inline-summary"><span>{data.category}</span><span>{routeLabel[routeState]}</span><span>{data.className === 'valve' ? (valveOpen ? 'Открыт' : 'Закрыт') : `${flow} л/мин`}</span><span>{isAlarmed ? 'Тревога' : blockedState ? 'Блокировка' : 'Норма'}</span></div>}
