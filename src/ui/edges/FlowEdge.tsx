...
const FlowEdgeComponent = ({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, data, selected }: EdgeProps) => {
  ...
  const { selectedEdgeId, edgeEditorMode, selectEdge, setEdgeEditorMode, executeEdgeAction } = useAppStore((state) => ({ selectedEdgeId: state.selectedEdgeId, edgeEditorMode: state.edgeEditorMode, selectEdge: state.selectEdge, setEdgeEditorMode: state.setEdgeEditorMode, executeEdgeAction: state.executeEdgeAction }));
  ...
  return <>
    ...
    <BaseEdge id={`${id}-pipe`} path={path} style={{ stroke: routeColor, strokeWidth: active ? 8 : 6, opacity: emphasis ? 1 : 0.8, strokeDasharray: mixedFlow ? '10 6' : undefined }} />
    <path d={path} className={`pipe-shell route-${routeKey} ${hovered ? 'is-hovered' : ''}`} style={{ stroke: routeColor }} />
    <path d={path} className={`pipe-glow ${active ? 'is-active' : ''} ${blocked ? 'is-blocked' : ''} ${isAlarm ? 'is-alarm' : ''}`} style={{ stroke: mixedFlow ? '#f3b6ff' : medium.glow }} />
    <path d={path} className={`pipe-fluid mode-${routeParticleMode[routeKey]} ${active ? 'is-active' : ''} ${blocked ? 'is-blocked' : ''} ${isStarved ? 'is-starved' : ''} ${isAlarm ? 'is-alarm' : ''}`} style={{ stroke: mixedFlow ? '#f0b7ff' : medium.base, ['--flow-speed' as string]: `${Math.max(0.7, 2.6 - Number(data?.flowRate ?? 0) / 36)}s` }} />
    <path d={path} className={`pipe-fluid pipe-fluid-secondary mode-${routeParticleMode[routeKey]} ${active ? 'is-active' : ''} ${blocked ? 'is-blocked' : ''} ${isStarved ? 'is-starved' : ''} ${isAlarm ? 'is-alarm' : ''}`} style={{ stroke: mixedFlow ? '#fff0ff' : medium.glow, ['--flow-speed' as string]: `${Math.max(0.9, 3.2 - Number(data?.flowRate ?? 0) / 44)}s` }} />
    <EdgeLabelRenderer>
